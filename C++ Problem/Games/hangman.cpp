...
int main(){
    ...
    for(int x = 0 ;; x++){
        scanf("%c", &temp) ;
        if(temp == ';')break;
        else if (temp == ' ') word.push_back(' ');
        else if (temp >= 'a' && temp <= 'z')word.push_back(temp - 32);
        else word.push_back(temp);
    }
    length = static_cast<int> (word.size());
    for(int x = 0; x < length ; x++) cout << word[x];
    ...
    while(!guessed){
        ...
        for(int x = 0; x < length ; x++){
            if(word[x]== ' ') cout << " ";
            else if(hit_arr[word[x] - 'A'] == 0 && detection[word[x] - 'A'] != 0) {cout << "-"; dashes++;}
            else if(hit_arr[word[x] - 'A'] != 0 && detection[word[x] - 'A'] != 0) cout << word[x];   
        }
        cout << endl;
        if(dashes == 0){
            guessed = true;
        ...
        if (fails == 6){
            ...
            for(int y = 0 ; y < length ; y++)cout << word[y];
            cout << endl;
        }
    }
